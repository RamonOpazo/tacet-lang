...
Ordinary evaluation produces a _value_ — an element of the value domain $V$. When evaluation instead encounters a computational phenomenon that is not an ordinary value — a possible failure, an unbounded recursion, an asynchronous wait, an intrinsically infinite loop — it raises an _obligation_: a thin, nominal signal whose entire content is its own identity, marking that a value is owed here and must be restored. Each obligation has a thick companion, its _witness_ — not a value but the _type_ that records what is owed, including the type of the value that was expected. The obligation is what the computation tracks and must discharge; the witness is what a function's interface advertises about it.

The `tacet` construct is the syntactic bridge that lets a computation raise such an obligation deliberately. The name is the musical direction _tacet_ — "it is silent," the instruction that an instrument plays nothing for a passage: where a value would sound, `tacet` leaves a rest. It silences a position where a value would sit, leaving an obligation in its place, and `reify` is what breaks the silence, restoring a value to the value space. Because an obligation is not a value, `tacet` names it with a value-level _symbol_ — a proxy that carries the obligation's meaning while the obligation itself stays outside the value space. Once an obligation is exposed, it must be _discharged_, and discharge is always _reification_. Usually it reifies the obligation back into an ordinary value; it may instead reify not into a value but into a _new_ obligation — deferring to a later step the decision of how to answer it, without ever leaving the discipline. Linearity governs the whole process: an obligation cannot be duplicated and cannot be silently dropped. Its only alternative to being discharged is an explicit, terminal failure — surfaced loudly, never absorbed into a quiet default or a silent bottom.
