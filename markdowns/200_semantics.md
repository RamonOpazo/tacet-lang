...
**(Capability.)** A function evaluates with $\varphi = \infty$ if and only if it declares the $\mathsf{Unbounded}_0$ capability in its signature; otherwise it evaluates with a finite budget $\varphi \in \mathbb{N}$. Thus the two regimes are one relation at two fuel settings, and the capability reads semantically as the lifting of the fuel bound. Because $\mathsf{Unbounded}_0$ is recorded in the signature, the choice of regime is disclosed to every caller.
...
In the default regime, $\varphi \in \mathbb{N}$ is finite. Because recursion and deferral strictly decrease $\varphi$ and every other step leaves it fixed, no configuration admits an infinite reduction sequence: fuel exhaustion converts would-be divergence into a finite reduction ending in a $\mathsf{Bound}$ obligation. Evaluation therefore always halts, $\mathrm{eval}$ is _total_ on $B_f$, and every branch reduces to some element of $R$. The fuel-exhaustion outcome is not $\bot$; it is an ordinary $\mathsf{Bound}$ obligation, typed as an $\mathsf{Unbounded}$ witness in $W$. Like every obligation, it is discharged through the single reification API (Section 2.5); its _default resolution_, $\mathsf{Bound}$, is precisely the fuel discipline just described. Section 2.2 fixes these per-kind default resolutions, and Section 2.5 the uniform reifiers that can override them.

In the renounced regime, $\varphi = \infty$, the fuel argument no longer forces termination, and reduction of such a branch may continue without end. This does _not_ reintroduce $\bot$. The possibility of non-termination is disclosed by the $\mathsf{Unbounded}_0$ capability in the signature; the branch is recorded by a null-obligation witness in $W_0$ — bookkeeping that owes nothing and ultimately maps to $\mathsf{Total}$ (Section 2.2). A computation that fails to halt produces no semantic value at all, but it produces no bottom either: $\bot$ remains outside $R$, and the risk it runs is exactly the one its capability announced. This is the governing principle in its sharpest form: honesty is the _disclosure of the risk_, never the production of $\bot$.
...
```{=latex}
\input{../figures/function_eval.latex}
```
...
& A
	\ar[d, "\operatorname{select}_f"]
& & L
	\ar[from=d, "\operatorname{name}"] \\
B
	\ar[from=r, hook]
& B_f
	\ar[from=rr, "p_B"]
& & L_f
	\ar[from=ddd, "\operatorname{expose}"'] \\
& R
	\ar[from=u, "\operatorname{eval}"]
	\ar[from=dl, hook]
	\ar[from=dr, hook] \\
V
	\ar[from=d, "\mathsf{Total}"]
& & W
	\ar[from=dl, hook]
	\ar[from=dr, hook] \\
\varnothing
...
<!--_Figure 2.1. Semantic structure of function evaluation._ An input selects a branch, $\operatorname{select}_f : A \to B_f$, which evaluates, $\operatorname{eval} : B_f \to R$, into the result domain $R = V \uplus W$ with $W = W_0 \uplus W_L$ (the hooked arrows are the coproduct injections). The three regions are the fibres of the disposition: $V$ holds $\mathsf{Total}$ outcomes — ordinary values, into which the empty witnessing type maps via $\mathsf{Total} : \varnothing \to V$ — while $W_L$ holds witnesses that expose a linear obligation and $W_0$ holds null-obligation witnesses, renounced bookkeeping that owes nothing and ultimately maps to $\mathsf{Total}$. Bottom is not an object of the diagram: $\bot \notin R$, unconditionally. On the obligation side, $L_f$ is the total space of the family $\operatorname{obligations} : B_f \to \mathbf{Set}$ (Section 2.3), carrying the projection $p_B : L_f \to B_f$ and the generally non-injective $\operatorname{name} : L_f \to L$; witnesses meet obligations through the set-valued $\operatorname{expose} : W_L \to \mathcal{P}_{\mathrm{fin}}(L_f)$. The internal structure of $V$, $W$, and $L_f$ is developed in Sections 2.2–2.3.-->

## Dispositions and their composition

### The disposition of an outcome

The partition of the result domain in Section 2.1 is not primitive; it is induced by the _disposition_ of a branch evaluation — the collection of computational phenomena its evaluation realizes. The default disposition is $\mathsf{Total}$, which is not a witness kind but the name of the _empty_ witnessing type: it realizes no phenomenon and exposes nothing, and for exactly that reason it maps to an ordinary value in $V$. The Church booleans $\mathsf{true} = x\,y \mapsto x$ and $\mathsf{false} = x\,y \mapsto y$ are ordinary such values — total functions that select between two values already held, raising nothing — and it is the empty witness $\mathsf{Total}$ that places them in $V$. The _decision_ axis of the language thus lives wholly in $V$, disjoint from the _absence_ axis, which lives wholly in $L$ and is reached only through $\mathsf{tacet}$; the two are never conflated in a single degenerate inhabitant.
...
This set is closed and exhaustive. Its four kinds are the orthogonal axes along which a computation can depart from producing a value — _partiality_, _boundedness_, _effectfulness_, and _loop productivity_ — and no further axis inhabits the space. The metatheory of Chapter 3 is defined over exactly these four.

Each witness exposes a named obligation, or none; the obligation it exposes determines the region of the result domain it inhabits:

| Witness                         | Exposes          | Domain |
| ------------------------------- | ---------------- | ------ |
| $\mathsf{Total}\,(\varnothing)$ | -                | $V$    |
| $\mathsf{Partial}$              | $\mathsf{Reify}$ | $W_L$  |
| $\mathsf{Unbounded}$            | $\mathsf{Bound}$ | $W_L$  |
| $\mathsf{Awaited}$              | $\mathsf{Sync}$  | $W_L$  |
| $\mathsf{Loop}$                 | $\mathsf{Break}$ | $W_L$  |
| $\mathsf{Unbounded}_0$          | -                | $W_0$  |
| $\mathsf{Awaited}_0$            | -                | $W_0$  |

The four linear kinds each expose an obligation and inhabit $W_L$. The obligation's name is its entire content: consistent with the thin-obligation ontology of Section 2.3, a $\mathsf{Bound}$ obligation records nothing but the nominal fact that an unboundedness occurred and must be answered; a $\mathsf{Reify}$ obligation, that a partiality occurred; and so on. The minus-marked witnesses $\mathsf{Unbounded}_0$ and $\mathsf{Awaited}_0$ are the _renounced_ forms, defined in the next subsection; they expose no obligation and inhabit $W_0$. $\mathsf{Total}$, the empty witnessing type, also exposes nothing — but unlike a $W_0$ witness it records no phenomenon at all, and so maps to $V$ rather than to $W$.

The partition of Section 2.1 now reads off the disposition, into three regions:

$$
\varnothing \to V, \qquad
\{\mathsf{Partial}, \mathsf{Unbounded}, \mathsf{Awaited}, \mathsf{Loop}\} \to W_L, \qquad
\{\mathsf{Unbounded}_0, \mathsf{Awaited}_0\} \to W_0 .
$$

A $\mathsf{Total}$ branch — the empty witness — lands in $V$; a branch exposing at least one obligation lands in $W_L$; a branch that is _witnessed but exposes no obligation_, renounced bookkeeping, lands in $W_0$. So $R = V \uplus W$ with $W = W_0 \uplus W_L$ — three genuine regions, not two.

### Renunciation

Two of the linear kinds admit a _renounced_ form, written with a superscript minus — the witness with its obligation removed:

$$
\mathsf{Unbounded}_0 \qquad \mathsf{Awaited}_0
$$

Renunciation removes the _obligation_ from a witness while retaining the witness itself. The superscript minus is category-theoretic notation: $\kappa_0$ names the non-obliged counterpart of the witness $\kappa$. An $\mathsf{Unbounded}_0$ or $\mathsf{Awaited}_0$ witness inhabits $W_0$: it is genuine bookkeeping — a record that the computation passed through a renounced-unbounded or fire-and-forget point — yet it exposes no obligation, by explicit declaration, and so imposes nothing to discharge. The phenomenon is not answered but _accepted_, and the acceptance is disclosed as a capability in the function's signature (Section 2.1). A $W_0$ witness carries no obligation and ultimately maps to $\mathsf{Total}$, hence into $V$, at the end of a computation (Section 3.6) — but it is a witness in $W$ throughout, never itself a value.

Because a renounced witness exposes no obligation, it is inert in the composition algebra, which constrains only _live_ linear kinds. A computation that is both failable and renounced-unbounded has live linear face $\{\mathsf{Partial}\}$; the renounced axis contributes no obligation, so no admissibility question arises. This is why the exclusions of the composition algebra (below) constrain only the pairs of live linear kinds — $\{\mathsf{Unbounded}, \mathsf{Awaited}\}$ and the like — and never a renounced form.
...
- $\mathsf{Unbounded}_0$ renounces the bound. Evaluation may run without end, disclosed by the capability; it is recorded as a $W_0$ witness — null-obligation bookkeeping, ultimately mapping to $\mathsf{Total}$ — and non-termination is never a bottom value.
- $\mathsf{Loop}$ exposes a $\mathsf{Break}$ obligation. A loop is intrinsically infinite: it does not draw on the fuel budget (a fuel-limited loop could not run forever, which would defeat the purpose of a productive one), and it iterates productively until its $\mathsf{Break}$ obligation is discharged. Uniquely among the kinds, a $\mathsf{Break}$ is discharged _out-of-band_ — by an explicit break originating outside the loop body, either an interrupt ($\mathtt{Ctrl}$-$\mathtt{C}$) or a call to a breaking function — rather than within the forward flow of evaluation. Until that break lands the branch does not complete; a loop never broken runs productively forever. This is not bottom: the branch simply never yields a value, and the $\mathsf{Break}$ obligation discloses, statically, that completion is contingent on an explicit break.
...
Admissibility is determined entirely by pairs and is downward closed, so the admissible dispositions are exactly the _cliques_ of the compatibility graph $G$ whose edges are the two surviving pairs, $\mathsf{Partial}$–$\mathsf{Unbounded}$ and $\mathsf{Partial}$–$\mathsf{Awaited}$. Thus $G$ is a two-leaf star centred on $\mathsf{Partial}$, together with an isolated $\mathsf{Loop}$. Because inadmissibility lives entirely in pairs, the admissible dispositions form a **flag (clique) complex**, with seven faces,
...
The two are bridged by exposure, and the direction of the asymmetry is exactly that a thick witness gives rise to thin obligations, never the reverse. A witness whose disposition is empty — a null-obligation $W_0$ witness (Section 2.2) — is thick all the same, carrying its payload type as bookkeeping, but gives rise to no obligation at all; exposure below concerns the $W_L$ witnesses, whose disposition is non-empty.
...
Obligations are not values: they live in $L$, outside the value space entirely, and so cannot be named, passed, or matched on directly in code. To let a program _refer_ to an obligation without ever _holding_ one, the language provides value-level proxies. A **symbol** is such a proxy — a value that stands in for an obligation as its avatar, carrying only the obligation's semantic meaning and none of its linear substance. A **symbol-construct** is a set of symbols. Symbols are what code manipulates; the obligations they name stay in $L$.

A symbol-construct is a signal to the type interpreter, which reads it as a type-level operation against $\Gamma$ or $\Delta$: a check on values, a declaration of witnesses, or the production of an obligation. Symbol-constructs serve several roles in the language; module loading and creation ($\mathsf{Load}$, $\mathsf{Module}$) are introduced in a later chapter. Here we are concerned with the two that bear on obligations: the `tacet` keyword, which _introduces_ obligations, and the function contract, which _declares_ witnesses.

The `tacet` keyword is the most universal of the obligation-introducing rules, because it is simply the introduction written directly in the code. Applied to a symbol-construct, `tacet` signals that the branch produces the linear obligation the symbols name, and introduces that obligation into $\Delta$. It can introduce _any_ obligation the language admits, with a single exception: it cannot introduce $\mathsf{Break}$. A $\mathsf{Break}$ is never raised by code — it is produced solely by a decision the user makes while the computation runs, an interrupt or a call to a breaking function — so no symbol introduces it. Every other obligation is within `tacet`'s reach.

The essential discipline is that `tacet` lands **purely in $L$**. It consumes a symbol — a value — and emits an obligation, which is not a value; the symbol is the avatar that carries the semantic meaning across this boundary, leaving the obligation itself wholly outside the value world. `tacet` never produces a witness. What it does do, beyond depositing the obligation in $\Delta$, is _force an implicit witness in the contract_: a branch that raises an obligation via `tacet` obliges the function's contract to carry the corresponding witness (below). Witnesses ($W$) thus remain the concern of the interface, obligations ($L$) the concern of the body, and `expose` (Section 2.3) is the map between them at the call boundary — the two domains never merge, which is exactly why they are kept separate. Writing the elaboration with a squiggle, to mark that it is not an ordinary semantic morphism,
...
\frac{\;\Gamma; \Delta \vdash e : T \qquad \mathrm{kinds}(\Delta) \cup \{\kappa\} \in \mathcal{X} \qquad \kappa_0 \notin \mathrm{caps}(f)\;}
     {\;\Gamma;\ \Delta,\, l : \mathsf{Witness}(T) \vdash \mathrm{intro}_\kappa(e) : \mathsf{Witness}(T)\;}
\quad (l\ \text{fresh})
$$

The rule covers the obligations `tacet` can introduce — $\mathsf{Reify}$ and $\mathsf{Bound}$ directly, and $\mathsf{Sync}$ through the effectful primitives; $\mathsf{Break}$ has no introduction rule and enters only through the contract. The admissibility premise is what makes the "impossible combinations" of Section 2.2 a typing fact rather than prose: an introduction that would leave the flag complex simply has no derivation. The capability premise $\kappa_0 \notin \mathrm{caps}(f)$ selects the _live_ case for the renounceable kinds $\mathsf{Unbounded}$ and $\mathsf{Awaited}$; the renounced case is the following rule.

When the corresponding capability is declared, the same introduction form is _transparent_ to the linear context: it adds no obligation to $\Delta$, recording instead a null-obligation witness in $W_0$ — bookkeeping that owes nothing and ultimately maps to $\mathsf{Total}$ (Section 2.2):

$$
\textbf{(T-Absorb}_\kappa\textbf{)}\qquad
\frac{\;\Gamma; \Delta \vdash e : T \qquad \kappa_0 \in \mathrm{caps}(f)\;}
     {\;\Gamma; \Delta \vdash \mathrm{intro}_\kappa(e) : T'\;}
\qquad \kappa \in \{\mathsf{Unbounded}, \mathsf{Awaited}\}.
$$

The result type $T'$ is $T$ for a renounced recursion ($\mathsf{Unbounded}_0$ in the contract), which yields the value it would have computed, and the trivial type for a renounced asynchronous launch ($\mathsf{Awaited}_0$), which returns immediately as a fire-and-forget sink. The linear context is unchanged: a $W_0$ witness exposes no obligation, so renunciation adds nothing to $\Delta$. Because asynchrony enters only through primitive effectful functions, $\mathsf{Awaited}_0$ renounces the $\mathsf{Sync}$ obligation those primitives would otherwise raise.

### The witness contract

Where `tacet` introduces _obligations_ in the body, a function's contract declares the _witnesses_ it exposes in its interface. The two are the syntactic faces of the thick/thin distinction of Section 2.3: obligations are the thin occurrences that live in $\Delta$; witnesses are the thick objects the signature advertises. A declaration may append a symbol-construct after the return type,

$$
f : A \to \tau \;\; \kappa_1 \cdots \kappa_n ,
$$

naming the witnesses the body exposes. This contract is the function's set of capabilities, and $\mathrm{caps}(f)$ is the subset of renounced symbols within it — those written $\kappa_0$, a witness with its obligation removed. The contract must _comply with the body_: the witnesses it declares must match the obligations the branches raise, up to renunciation. Left empty, it is inferred — the function is linearly typed by the evaluation of its branches. The intent is to be _always explicit about obligations_, which the body's `tacet`s make unavoidable, while _opting in to witnesses_, which the contract makes a deliberate part of the interface, since the witness API is itself something the programmer defines.

$\mathsf{Loop}$ is the one witness that can _only_ be declared, never introduced. Because its $\mathsf{Break}$ cannot be raised in code, a loop reaches the type system solely by adding $\mathsf{Loop}$ to the contract, which must be made explicit and must comply with the function's main branch. This matches the isolation of $\mathsf{Loop}$ in the flag complex (Section 2.2): a loop stands alone as a witness, separated from every other, so a $\mathsf{Loop}$ contract admits no companion. Renunciation is declared the same way — $\mathsf{Unbounded}_0$ or $\mathsf{Awaited}_0$ in the contract absorbs the corresponding obligation into $\mathsf{Total}$ (Section 2.2), which is how a programmer opts into possible unboundedness or a fire-and-forget effect.
